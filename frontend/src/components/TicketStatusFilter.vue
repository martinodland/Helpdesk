...
<template>
  <div class="grid grid-cols-2 lg:grid-cols-4 w-full bg-(--main-background) p-1 rounded-lg gap-1 mb-6 font-bold w-fit">
    ...
    <div v-on:click="router.push({ query: { ...route.query, status: 'InProgress' } }); filterTickets('InProgress')" class="flex flex-row gap-2 bg-white px-4 py-2 rounded-lg items-center cursor-pointer">
    ...
    <div v-on:click="router.push({ query: { ...route.query, status: 'Closed' } }); filterTickets('Closed')" class="flex flex-row gap-2 bg-white px-4 py-2 rounded-lg items-center cursor-pointer">
